  ...
  return <DashboardShell><div className="mx-auto max-w-5xl px-4 py-8 sm:px-8">
    ...
    {loading ? <p className="py-8 text-center text-sm text-stone-400">Carregando convidados...</p> : invitations.length === 0 ? <Card className="p-8 text-center"><Users className="mx-auto mb-3 text-stone-300" /><p className="text-sm text-stone-500">Nenhum convite criado ainda.</p></Card> : visibleInvitations.length === 0 ? <Card className="p-8 text-center"><p className="text-sm text-stone-500">Nenhum convidado neste filtro.</p></Card> : <div className="space-y-2">{visibleInvitations.map(invitation => <Card key={invitation.id} className="p-3 sm:p-4">
      <div className="relative flex min-w-0 items-center justify-between gap-2">
        <div className="min-w-0"><h3 className="truncate font-medium text-stone-900" title={invitation.displayName}>{invitation.displayName}</h3><p className="truncate text-[11px] text-stone-400">{invitation.guests.length} pessoa(s) {statusFilter !== 'ALL' ? 'neste filtro' : ''} · Código: <strong className="font-mono font-semibold tracking-wider text-stone-600">{invitation.accessCode}</strong></p></div>
        <div className="flex flex-shrink-0 items-center gap-1"><button type="button" onClick={() => copyInvite(invitation)} className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border border-stone-200 px-2.5 text-xs font-medium text-stone-600 transition-colors hover:bg-stone-50">{copiedInvitationId === invitation.id ? <Check size={14} className="text-emerald-600" /> : <Copy size={14} />} {copiedInvitationId === invitation.id ? 'Copiado' : 'Copiar mensagem'}</button><button type="button" onPointerDown={event => event.stopPropagation()} onClick={() => setOpenInvitationMenuId(current => current === invitation.id ? null : invitation.id)} className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-stone-500 transition-colors hover:bg-stone-100" aria-label={`Outras ações do convite ${invitation.displayName}`} aria-expanded={openInvitationMenuId === invitation.id}><MoreVertical size={17} /></button></div>
        {openInvitationMenuId === invitation.id && <div onPointerDown={event => event.stopPropagation()} className="absolute right-0 top-10 z-30 min-w-48 rounded-xl border border-stone-200 bg-white p-1.5 shadow-lg"><button type="button" onClick={() => copyConfirmationCode(invitation)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-xs font-medium text-stone-700 hover:bg-stone-50">{copiedCodeId === invitation.id ? <Check size={14} className="text-emerald-600" /> : <Copy size={14} />} {copiedCodeId === invitation.id ? 'Código copiado' : 'Copiar apenas código'}</button><button type="button" onClick={() => remove(invitation.id)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-xs font-medium text-red-500 hover:bg-red-50"><Trash2 size={14} /> Remover convite</button></div>}
      </div>
